namespace UnityEngine.Rendering.Universal
{
    public class PostProcessUrpPass : ScriptableRenderPass
    {
        ...
        private RenderTargetIdentifier faxxCopy = new RenderTargetIdentifier(faxxString);

        private bool maskSet = false;
        private int numberOfPasses = 3;
        private readonly string tag;
        private readonly bool faxx;
        ...
        static readonly int faxxString = Shader.PropertyToID("_Faxx");
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            if(faxx)
            {
                cmd.GetTemporaryRT(faxxString, opaqueDesc, FilterMode.Bilinear);
                cmd.CopyTexture(source, faxxCopy);
                cmd.Blit(faxxCopy, tempCopy, material, 0);
            }
            else
            {
                ...
                //cmd.Blit(faxxCopy, tempCopy, material, 3);
            }
            ...
            if (bloom || blur)
            {
                ...
                if (bloom)
                {
                    ...
                    float soft = bloomSoftness <= 0.00001f ? 0.001f : bloomSoftness;
                    knee                        = bloomThreshold * soft;
                    material.SetVector(blDataString, new Vector4(bloomThreshold, bloomThreshold - knee, 2f * knee, 1f / (4f * knee + 0.00001f)));
                }
                if (blur)
                {
                ...
                if (blurAmount > 0 || !blur)
                {
                    ...
                    else if (numberOfPasses == 3)
                    {
                        ...
                        cmd.Blit(tempCopy,  blurTex,    material, 1);
                        cmd.Blit(blurTex,   blurTemp,   material, 1);
                        cmd.Blit(blurTemp,  blurTex,    material, 1);
                    }
