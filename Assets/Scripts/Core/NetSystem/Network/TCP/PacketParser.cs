					...
					else
					{
						...
						switch (this.packetSizeLength)
						{
							case MessagePacket.PacketSizeLength4:
								...
								if (this.packetSize > ushort.MaxValue * 16 || this.packetSize < MessagePacket.MinPacketSize)
								{
							...
							case MessagePacket.PacketSizeLength2:
								...
								if (this.packetSize > ushort.MaxValue || this.packetSize < MessagePacket.MinPacketSize)
								{
									throw new Exception($"recv packet size error:, 可能是外网探测端口: {this.packetSize}");
								}
								break;
