...
{
    [CompilerGenerated]
    private static Dictionary<string, int> s_Keys;
        ...
        if (current.type != EventType.Used)
        {
            ...
            switch (current.GetTypeForControl(controlID))
            {
                ...
                case EventType.ExecuteCommand:
                {
                    ...
                    if (!s_Keys.TryGetValue(commandName, out num15) || (num15 != 0))
                    {
                        break;
                    }
                    current.Use();
    ...
    public static void HandleUtility_handleWireMaterial_SetPass(int pass)
    {
        Type _type = typeof(HandleUtility);
        var _member = _type.GetProperty("handleWireMaterial", 
            System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.GetProperty);
        var _value = (Material)_member.GetValue(null, null);
        _value.SetPass(pass);
    }
