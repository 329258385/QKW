...
public static class NGUIEditorTools
{
	...
	/// 绘画list对象
	/// </summary>
	static public SerializedProperty DrawPropertyList(string label, SerializedObject serializedObject, string property, string[] subProperty, params GUILayoutOption[] options)
	{
		...
		if (sp != null) {
			...
			if (sp.isArray && sp.type != "string") {
				...
				DrawList(serializedObject, sp, property, label ?? property, subProperty);
			}
	...
	/// 绘画list
	/// </summary>
	static public SerializedProperty DrawList(this SerializedObject obj, SerializedProperty parent, string property, string title, string[] subProperty)
	{
		...
		if (sp != null && NGUIEditorTools.DrawHeader (title)) {
			...
			{
				SerializedProperty pro = parent.GetArrayElementAtIndex (i);

				if(pro != null) EditorGUILayout.PropertyField(pro);

				if (subProperty != null || subProperty.Length != 0) {

					for (int k = 0; k < subProperty.Length; k++) {

						SerializedProperty p = pro.FindPropertyRelative (subProperty[k]);

						if(p != null) EditorGUILayout.PropertyField(p, new GUIContent(subProperty[k]), GUILayout.MinWidth(20f));
